...
def test_name(diff):
    ...
    assert diff.name() == "origin/master...HEAD, staged and unstaged changes"
...
def test_name_include_untracked(git_diff):
    ...
    assert (
        ...
        == "origin/master...HEAD, staged, unstaged and untracked changes"
    )
...
def test_name_with_default_range(git_diff):
    reporter = GitDiffReporter(git_diff=git_diff, ignore_staged=True)
    assert reporter.name() == "origin/master...HEAD and unstaged changes"
